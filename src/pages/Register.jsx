        ...
        <section className='min-h-screen w-full flex items-center justify-center py-5'>
            <div className="container flex flex-wrap mx-auto px-5">
                ...
                <div className='md:w-1/2 w-full flex flex-col gap-10'>
                    ...
                    <h2 className='text-3xl font-semibold'>Join today.</h2>

                    <form className='flex flex-col gap-3 md:w-1/2 w-full text-lg text-gray-800' onSubmit={handleSubmit}>
                        <input type="text" placeholder='Enter Name' value={name} onChange={(e) => setName(e.target.value)} className='rounded-full border  py-2 px-6' required />
                        ...
                        <button className='bg-indigo-500 text-white  rounded-full py-2 px-6 hover:bg-indigo-600'>{loading? 'Loading...':"Create Account"}</button>
                        <p>Already have an Account ?</p>
                        <Link to={"/login"} className='border text-md rounded-full py-2 px-6 hover:bg-gray-200 text-center'>Sign In</Link>
                    </form>
